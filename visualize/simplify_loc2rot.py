...
class joints2smpl:
    ...
    def npy2smpl(self, npy_path):
        ...
        motions = {}
        motions['motion'] = np.load(npy_path, allow_pickle=True)#[None][0]
        #print_batch('', motions)
...
if __name__ == '__main__':
    ...
    a = np.load(params.input_path)
    print(a.shape)
    simplify = joints2smpl(a.shape[1],device_id=params.device, cuda=params.cuda)
    ...
    if os.path.isfile(params.input_path) and params.input_path.endswith('.npy'):
        ...
        files = ["./como_diversity/redo_joint.npy"]#os.path.join(params.input_path, f) for f in os.listdir(params.input_path) if f.endswith('.npy')]
        for f in files:
            simplify.npy2smpl(f)