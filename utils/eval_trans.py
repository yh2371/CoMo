...
@torch.no_grad()        
def evaluation_dec(out_dir, val_loader, net, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, eval_wrapper, draw = True, save = True, savegif=False, savenpy=False, unit_length=4) : 
    net.eval()
    nb_sample = 0
    ...
    for batch in tqdm(val_loader):
        ...
        for i in range(bs):
            ...
            pred_pose = net(code_indices[i:i+1,:m_length[i]:unit_length].cuda().float()) 
            pred_denorm = val_loader.dataset.inv_transform(pred_pose.detach().cpu().numpy())
            pred_xyz = recover_from_ric(torch.from_numpy(pred_denorm).float().cuda(), num_joints)
...
@torch.no_grad()        
def evaluation_transformer(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, draw = True, save = True, savegif=False, unit_length = 4): 
    ...
    for i in range(1):
        for batch in tqdm(val_loader):
            ...
            for k in range(bs):
                ...
                index_motion = trans.sample(feat_clip_text[k:k+1], False, m_length[k]//unit_length)[:,:,:-2] # 1 x t x code_num
            ...
            et_pred, em_pred = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, pred_pose_eval, pred_len)#m_length)

            if i == 0:
                pose = pose.cuda().float()
                
                et, em = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, pose, m_length) #m_length
                motion_annotation_list.append(em)
                motion_pred_list.append(em_pred)
...
@torch.no_grad()        
def evaluation_transformer_test(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, best_multi, clip_model, eval_wrapper, draw = True, save = True, savegif=False, savenpy=False, unit_length = 4) : 
